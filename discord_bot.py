...
def ensure_qdrant_collection(collection_name="mem0_default", vector_size=768):
    ...
    try:
        ...
        if existing_vector_size != vector_size:
            logger.info(f"Collection exists but with wrong dimensions: {existing_vector_size}. Recreating...")
...
class ChatAgent:
    ...
    async def chat(self, query: str, user_id: str) -> str:
        ...
        """
        try:
            # 1. Retrieve relevant memories with user_id filter
            ...
            # 2. Prepare context with memories
...
Please respond to the user naturally, taking into account any relevant previous conversation context.
"""
    ...
    try:
        ...
        async with ctx.typing():
            ...
            if category == "KNOWLEDGE_BASE":
                ...
                # Pass context to the web search agent
                ...
                final_report = await loop.run_in_executor(
                    None,
                    lambda: run_parallel_research(query, context)
                )
...
async def force_search(ctx, *, query):
    ...
    try:
        ...
        # Get context and use web search agent
...
async def force_research(ctx, *, research_goal):
    ...
    try:
        ...
        final_report = await loop.run_in_executor(
            None,
            lambda: run_parallel_research(research_goal, context)
        )
        await send_chunked_message(ctx, final_report)
